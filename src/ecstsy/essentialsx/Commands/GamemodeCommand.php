...
class GamemodeCommand extends BaseCommand {
    ...
    public function onRun(CommandSender $sender, string $aliasUsed, array $args): void {
        ...
        if (isset($args["gamemode"])) {
            ...
            $player = isset($args["name"]) ? Utils::getPlayerByPrefix($args["name"]) : $sender->getName();
            ...
            $sender->sendMessage(C::colorize(str_replace(["{player}", "{gamemode}"], [$player->getName(), $gamemode->getEnglishName()], $config->getNested("gamemode.set_gamemode", "&aSet {target_player}'s game mode to {gamemode}."))));
        }
    }
